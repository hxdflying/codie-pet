...
9. Run the installed build script from this plugin root with `--workspace <current workspace>`.
10. Run the installed validation script from this plugin root with `--workspace <current workspace>`.
11. Ask the user to review `codie-pet/previews/preview.html` or `codie-pet/previews/contact-sheet.png`.
12. If the user approves installation, run the installed rules installer from this plugin root with `--workspace <current workspace>`.
...
Resolve scripts relative to the installed plugin root. Do not assume the current workspace contains this repository.

This skill file lives under `<installed plugin root>/skills/codie-pet/SKILL.md`. The scripts live under `<installed plugin root>/scripts/`:
...
Use the available Python launcher for the platform:

- macOS/Linux: `python3 <installed plugin root>/scripts/build_avatar_pack.py --workspace <current workspace>`
- Windows: `py <installed plugin root>\\scripts\\build_avatar_pack.py --workspace <current workspace>`
...
- **`build_avatar_pack.py` reports an unsupported PNG feature** (for example interlaced PNG or non-8-bit PNG). Regenerate that state strip as a normal 8-bit, non-interlaced PNG and retry.
